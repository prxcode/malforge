...
class Volatility3Adapter:
    ...
    def check_availability(self) -> tuple[bool, str]:
        ...
        if not hasattr(self, 'is_configured') or not self.is_configured:
            return False, "Volatility 3 failed to initialize (missing ISF symbols or valid memory image layer)."
    ...
    def _run_pslist_mock(self) -> list[dict]:
        ...
        return [
            {"pid": 4, "ppid": 0, "name": "System", "offset": "0x12345678", "threads": 120, "handles": 500, "session_id": None},
            {"pid": 344, "ppid": 4, "name": "smss.exe", "offset": "0x23456789", "threads": 3, "handles": 40, "session_id": None},
            {"pid": 468, "ppid": 344, "name": "csrss.exe", "offset": "0x3456789a", "threads": 12, "handles": 400, "session_id": 0},
            {"pid": 2340, "ppid": 468, "name": "cmd.exe", "offset": "0x456789ab", "threads": 1, "handles": 30, "session_id": 1},
        ]

    def _run_netscan_mock(self) -> list[dict]:
        """Mock netscan output for demonstration."""
        return [
            {"protocol": "TCP", "local_addr": "192.168.1.100", "local_port": 4444, "remote_addr": "185.20.10.5", "remote_port": 80, "state": "ESTABLISHED", "pid": 2340, "owner": "cmd.exe"},
            {"protocol": "UDP", "local_addr": "0.0.0.0", "local_port": 137, "remote_addr": "*", "remote_port": "*", "state": "", "pid": 4, "owner": "System"},
        ]

    def _run_malfind_mock(self) -> list[dict]:
        """Mock malfind output for demonstration."""
        return [
            {"pid": 2340, "process": "cmd.exe", "start": "0x7fff0000", "end": "0x7fff1000", "tag": "VadS", "protection": "PAGE_EXECUTE_READWRITE", "hex_dump": "4d 5a 90 00 03 00 00 00"}
        ]
